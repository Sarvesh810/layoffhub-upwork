...
from .serializers import UpdateProfileSerializer, \
    ...
    UserProfileSerializer, PasswordResetRequestSerializer, PasswordResetSerializer, CompanyQuestionCountSerializer
...
class FilterQuestionsByCompanyView(generics.ListAPIView):
    serializer_class = CompanyQuestionCountSerializer
    permission_classes = [AllowAny]

    def get_queryset(self):
        company_name = self.kwargs.get('company_name')

        # Annotate each company with the count of associated questions
        return (
            Question.objects
            .filter(companies__name__icontains=company_name)
            .values('companies__name')  # Group by company name
            .annotate(question_count=Count('id'))  # Count questions per company
        )
